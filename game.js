...
function unlockAudio() {
    ...
    tryPlayMusic();
    // Tocar sons que estavam pendentes
    pendingSounds.forEach(n => playSound(n));
    pendingSounds = [];
    // Se o título inicial é shock, tocar agora
...
// ─── GUARDAR RESULTADOS ──────────────────────────────────────────────────────
// Tenta enviar para o servidor local (server.js). Se não estiver disponível,
// guarda em localStorage como backup.
async function saveResult(nome, turma, tempo_ms) {
    ...
    // 1) Tentar POST para o servidor local
    try {
        ...
        if (resp.ok) return; // guardado no servidor ✓
    } catch(e) {
        // servidor não disponível — usar localStorage como backup
    }

    // 2) Backup em localStorage
    try {
...
function handleClick(x, y) {
    ...
    if (state === "menu") {
        ...
        } else if (hitTest(ID_BTN_VOLTAR)) state = "menu";
