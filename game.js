    ...
    if (m) {
        ...
        const tryPlay = () => {
            m.play().then(() => {
                ...
                console.log("⚠️ Tentativa falhada, a tentar novamente...");
...
const SHEETS_URL = "https://script.google.com/macros/s/AKfycbyp0QIBVBRofzWFTwwSSEHisV7o18UnQ7G_gRpI3jbj3NPYYRdDiLyb9GaGDsDYVyen/exec";
// ──────────────────────────────────────────────────────────────────────────────
...
function gameLoop() {
    ...
    // Garantir que a música está sempre a tocar (importante em telemóveis)
    if (audioUnlocked) {
        const m = sounds["bg_music"];
        if (m && m.paused) {
            m.play().catch(() => {});
